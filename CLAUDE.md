...
| 2026-06-04 | Claimed brightness was inaccessible / Apple-locked-down — wrong. `DisplayServicesGetBrightness` works via `dlopen` from dyld shared cache. | Do not declare an API impossible without exhausting `dlopen` + `dlsym` from the shared cache, especially for private frameworks on Apple Silicon. |
| 2026-06-04 | Claimed CoreAudio was "decoupled" on Tahoe and writes silently no-op — wrong. Writes work fine. The real bug was no settability check on read-only scalars. | Never declare a framework broken without testing write + readback verification (osascript, sysctl, etc.). Correlate the symptom with the real cause before concluding. |
| 2026-06-04 | Elided `/tmp` probe code as `...` in summaries — unreviewed code that tripped privacy prompts. | Do not summarize code out of diffs. Every line the user hasn't seen is a risk. Paste or explain fully. |
| 2026-06-04 | "Done" and "verified" diverged multiple times in the optimistic direction. | Do not close a task on self-assessment. Ground truth is: a test passed, a file readback matches, a fresh clone builds. The agent's own "it works" is not evidence. |
